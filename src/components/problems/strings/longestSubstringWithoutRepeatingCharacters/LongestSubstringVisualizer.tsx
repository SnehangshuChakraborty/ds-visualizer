...
import { PRESETS, BOUNDARY_LINES, BOUNDARY_CONDITIONS } from './constants/longestSubstringData';
import longestSubstringJava from '../java_codes/LongestSubstring.java?raw';
import { generateSteps } from './utils/longestSubstringGenerator';
import { useInterval } from '../../../../hooks/useInterval';
import './LongestSubstringVisualizer.css';

interface LongestSubstringVisualizerProps {
  celebrate: () => void;
}

export const LongestSubstringVisualizer: React.FC<LongestSubstringVisualizerProps> = ({
  celebrate,
}) => {
  ...
  const [customInputText, setCustomInputText] = useState('abcabcbb');

  // Interactive tip/accordion states
  const [accordionOpen, setAccordionOpen] = useState(true);
  const [activeBoundaryTip, setActiveBoundaryTip] = useState<number | null>(null);

  // If input string changes, reset tracking states automatically
  ...
  // Celebrate with fireworks when trace reaches final success frame
  useEffect(() => {
  ...
  // Generate a random string with duplicate characters
  const generateRandom = () => {
    const pool = 'abcdefg'; // small pool increases duplicates rate naturally!
    const length = 6 + Math.floor(Math.random() * 7); // 6 to 12 chars
    let randomStr = '';
    for (let i = 0; i < length; i++) {
      randomStr += pool.charAt(Math.floor(Math.random() * pool.length));
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
      ...
      >
        <div className="game-title" style={{ color: 'var(--theme-accent)' }}>
          <span>🫧</span>
          <span>The Sliding HashSet Bubble Window Sandbox</span>
        </div>
        <div className="game-stats">
          ...
          <div className="stat-item" style={{ borderColor: 'rgba(6,182,212,0.2)' }}>
            <span>Algorithm:</span>
            <strong style={{ color: '#06b6d4' }}>Sliding Window</strong>
          </div>
        </div>
      </section>
